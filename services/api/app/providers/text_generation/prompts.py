...
STORY_RULES = """
Одна публикация сразу на все площадки (Instagram, VK, Яндекс.Карты) — универсальный текст без платформенных фишек.
caption — мини-история вокруг блюда: один цепляющий интересный факт (происхождение блюда, необычный ингредиент,
кулинарная деталь, о которой мало кто знает) + живое аппетитное описание. Факт должен быть правдоподобным и конкретным.
hook — первая фраза истории, которая останавливает скролл.
voice_script — закадровый текст ролика до 15 секунд по этой же истории: факт → блюдо → приглашение.
hashtags — 3-5 универсальных, без спама.
""".strip()


PLATFORM_RULES = {
    "all:story": STORY_RULES,
...
def build_scene_plan_user_prompt(
    ...
    return (
        "Составь план сцен для одного вертикального ролика — мини-истории вокруг блюда. "
        "Ролик строится на истории из draft (интересный факт о блюде): "
        f"ровно {scenes_count} сцен — хук с цепляющим фактом в первой, развитие истории и показ блюда в середине, CTA в последней. "
        "voice_text сцен вместе должны складываться в связный рассказ, а не повторять одну фразу. "
