...
Current step: Stage 12 complete. The pipeline is two-stage: texts + scenario are approved first (free to regenerate), and only then the paid AI-video generation runs once, followed by a final video approval and publication. The dashboard was redesigned into a light minimal UI. Runtime still uses mock until `AI_VIDEO_FAL_KEY` is provided.

Approval flow: `Upload -> analysis -> drafts + scene plan (text only) -> content approval -> produce_video_stage (fal scenes + voice + assembly) -> video approval -> publication tasks`.
...
- fal.ai AI-video adapter (`AI_VIDEO_PROVIDER=fal`): uploads the dish photo to fal storage, submits image-to-video jobs to the queue API, polls, downloads results. Model switched via `AI_VIDEO_FAL_MODEL` (Seedance/PixVerse/Kling/Hailuo on one key).
- AI-video provider fallback: missing key or runtime failure falls back to mock when `ENABLE_PROVIDER_FALLBACK=true`; fallback reason recorded in scene `raw_response`.
- Scene generation now passes the source dish photo bytes to the AI-video provider for image-to-video conditioning.
- `TextGenerationProvider.generate_scene_plan()`: real scene plans via OpenRouter (JSON-validated, normalized), mock provider keeps the previous 4-beat template; `ScenePlan.metadata_json.provider` records the actual planner.
- Provider diagnostics dashboard now includes the `ai_video` area (fal/kling/runway keys, fallback state).
- Production guardrails accept `AI_VIDEO_PROVIDER=fal` and require `AI_VIDEO_FAL_KEY`.
- Two-stage approval: `ApprovalTask.stage` (`content`/`video`) with migration; stage 1 stops after texts + LLM scenario (no paid generation), approving it enqueues `produce_video_stage`, approving the video creates publication tasks. Telegram messages describe each stage.
...
- Provide a real `AI_VIDEO_FAL_KEY` and run one paid smoke (Seedance or PixVerse) to validate the fal adapter end-to-end and check per-clip cost.
- Hybrid economy mode: ffmpeg camera motion over the real dish photo + 1-2 AI hero clips (target ~$0.2-0.6 per reel).
- Add Telegram actions for regenerate scene/full AI video.
- Character reference frame for cross-scene mascot consistency (currently text-prompt only).
- Instagram real publishing (requires a Business account + Graph API); Yandex Maps stays a manual package.
- VK video upload paid smoke with a real user token (`video` scope) once VK credentials are provided.

## Blockers

- Real fal.ai AI-video requires `AI_VIDEO_FAL_KEY` (one key covers Seedance/PixVerse/Kling models via `AI_VIDEO_FAL_MODEL`).
...
- 2026-07-02: Added fal.ai AI-video adapter with storage upload, queue submit/poll, result download, and per-plan reference-image cache; factory now supports `fal` with mock fallback.
- 2026-07-02: Scene generation passes source dish photo bytes for image-to-video; scene plans generated via `TextGenerationProvider.generate_scene_plan()` (OpenRouter real, mock template preserved).
- 2026-07-02: Added `ai_video` provider diagnostics area, fal production guardrails, and new env vars (`AI_VIDEO_FAL_KEY`, `AI_VIDEO_FAL_MODEL`, poll timeouts) to env examples.
- 2026-07-03: Two-stage approval shipped (ApprovalTask.stage + migration): scenario approved before paid video generation; produce_video_stage Celery task; publication after video approval.
