...
Current step: Stage 11 complete, real AI-video generation is wired through fal.ai (`AI_VIDEO_PROVIDER=fal`) and scene plans are generated by the text provider (OpenRouter with mock fallback). Runtime still uses mock until `AI_VIDEO_FAL_KEY` is provided.
